...
/*modified by 6f6626 to work for sun3i
 *
 */
...
/* check sum functon from sun3i boot code */
...
int gen_check_sum(void *boot_buf){
  ...
  int len;
  int loopfor;
  struct boot_file_head *head_p=(struct boot_file_head *)boot_buf;
  head_p->check_sum = 0x5f0a6c39;
  if((head_p->length&0x03)!=0)
   return -1;
  len = head_p->length >> 2;
  compCksum = 0;
  readPtr = boot_buf;
  do {
    ...
    readPtr3 = readPtr + 3;
    readPtr = readPtr + 4;
    compCksum = *readPtr3 + *readPtr2 + *readPtr1 + tmp + compCksum;
    len = len - 4;
  } while (3 < len);
  while (loopfor = len != 0, len = len - 1, loopfor) {
    compCksum = *readPtr + compCksum;
    readPtr = readPtr + 1;
  }
  head_p->check_sum = compCksum;
  return 0;
}
...
int main(int argc, char *argv[])
{
	...
	memset(img.pad,0x00,BLOCK_SIZE);
	/* fill the header */
